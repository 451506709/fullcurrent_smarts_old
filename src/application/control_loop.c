...
static uint32 motorOffsetIndex = 0;
...
static void a2dPortal( int argc, char** argv );
static void drivemodePortal( int argc, char** argv );
static void PPOffsetAnglePortal( int argc, char** argv );
static void PrintHsLog( int argc, char** argv );
...
static void PrintHsLog( int argc, char** argv )
{
	for ( int i = 0; i < HS_LOG_DATA_COUNT; i++ )
	{
		printf("%0.2f %0.2f %0.2f %0.2f %0.2f %0.2f\n",
				hsLog[(i+hsLogIndex)%HS_LOG_DATA_COUNT].id,
				hsLog[(i+hsLogIndex)%HS_LOG_DATA_COUNT].iq,
				hsLog[(i+hsLogIndex)%HS_LOG_DATA_COUNT].motorPos,
				hsLog[(i+hsLogIndex)%HS_LOG_DATA_COUNT].vbus,
				hsLog[(i+hsLogIndex)%HS_LOG_DATA_COUNT].vq,
				hsLog[(i+hsLogIndex)%HS_LOG_DATA_COUNT].vd);
...
/************************************************************/
/************** DEBUGGING / DATA DUMP CODE ******************/
/************************************************************/

static CanTxMsg voltageAB;
static CanTxMsg voltageC;

static CanTxMsg currentAB;
static CanTxMsg currentC;

static CanTxMsg currentDQ;
static CanTxMsg voltageDQ;

static CanTxMsg voltage12;
static CanTxMsg voltage3;

static CanTxMsg position;
static CanTxMsg Vbus_Vgate;

static CanTxMsg calibration;
static CanTxMsg temperature;

void InitDebugCanPacket( CanTxMsg *m, uint8_t DLC )
{
	m->RTR = CAN_RTR_Data;
	m->DLC = DLC;
}
...
static void DetermineMotorPositionOffsets( void )
{
	...
	for ( uint32 i = 0; i < MOTOR_OFFSET_DELTA_SAMPLE_SIZE; i++ )
	{
...
static void SendDebugData( void * pvParameters)
{
	...
	analogReadings.Vb = 0;
	analogReadings.Vc = 0;
	analogReadings.Vbus = 0;
	analogReadings.Vgate = 0;

	InitDebugCanPacket(&voltageAB, 8);
	InitDebugCanPacket(&voltageC,  4);
	InitDebugCanPacket(&currentAB, 8);
	InitDebugCanPacket(&currentC,  4);
	InitDebugCanPacket(&currentDQ, 8);
	InitDebugCanPacket(&voltageDQ, 8);
	InitDebugCanPacket(&voltage12, 8);
	InitDebugCanPacket(&voltage3,  4);
	InitDebugCanPacket(&position,  8);
	InitDebugCanPacket(&Vbus_Vgate,  8);
	InitDebugCanPacket(&calibration, 8);
	InitDebugCanPacket(&temperature,   8);
	...
	while (1)
	{
		if ( motorOffsetMode )
		{
			if ( motorOffsetIndex < MOTOR_OFFSET_DELTA_SAMPLE_SIZE )
			{
				count++;

				if ( count >= MOTOR_OFFSET_DELTA_PERIOD_MS / DEBUG_PERIOD_MS )
				{
					count = 0;

					LogMotorOffsetDataPoint();
				}
			}
			else
			{
				count = 0;
				motorOffsetMode = false;

				DetermineMotorPositionOffsets();
			}
		}

		float fieldStrength = mCAN_GetPolePositionFieldStrength();

		memcpy(&(voltageAB.Data[0]), &(fieldStrength), sizeof(float));
		memcpy(&(voltageAB.Data[4]), &(analogReadings.Vb), sizeof(float));
		memcpy(&( voltageC.Data[0]), &(analogReadings.Vc), sizeof(float));

		memcpy(&(currentAB.Data[0]), &(analogReadings.Ia), sizeof(float));
		memcpy(&(currentAB.Data[4]), &(analogReadings.Ib), sizeof(float));
		memcpy(&( currentC.Data[0]), &(analogReadings.Ic), sizeof(float));

		//memcpy(&(voltage12.Data[0]), &(foc.V1), sizeof(float));
		//memcpy(&(voltage12.Data[4]), &(foc.V2), sizeof(float));
		//memcpy(&( voltage3.Data[0]), &(foc.V3), sizeof(float));

		memcpy(&(voltageDQ.Data[0]), &(foc.Vd), sizeof(float));
		memcpy(&(voltageDQ.Data[4]), &(foc.Vq), sizeof(float));

		//analogReadings.Vgate = adc_GetReading(ADC_CH_INDEX_VGATE, FILTER, FILTER_LEN);

		memcpy(&(Vbus_Vgate.Data[0]), &(analogReadings.Vbus), sizeof(float));
		memcpy(&(Vbus_Vgate.Data[4]), &(analogReadings.Vgate), sizeof(float));

		memcpy(&(currentDQ.Data[0]), &(foc.Id), sizeof(float));
		memcpy(&(currentDQ.Data[4]), &(foc.Iq), sizeof(float));

		float w = MotorPosition_GetMechanicalVelocity();
		float theta = WrapAngle(foc.theta);

		memcpy(&(position.Data[0]), &(theta), sizeof(float));
		memcpy(&(position.Data[4]), &(w), sizeof(float));

		fullCAN_SendPacket(&position,  FULLCAN_IDBASE_POSITION);
		fullCAN_SendPacket(&voltageDQ, FULLCAN_IDBASE_VOLTAGE_DQ);
		fullCAN_SendPacket(&currentDQ, FULLCAN_IDBASE_CURRENT_DQ);
		fullCAN_SendPacket(&Vbus_Vgate,FULLCAN_IDBASE_VBUS_VGATE);
		fullCAN_SendPacket(&voltageAB, FULLCAN_IDBASE_VOLTAGE_AB);
		fullCAN_SendPacket(&voltageC,  FULLCAN_IDBASE_VOLTAGE_C);
		calPktDelay++;

		if ( calPktDelay >= 5000/DEBUG_PERIOD_MS)
		{
			calPktDelay = 0;

			calibration.Data[0] = motorOffsetMode;
			calibration.Data[1] = 0xFF;
			calibration.Data[2] = swapAAndBPhases;
			calibration.Data[3] = MotorPosition_GetPositionInversion();
			float off = MotorPosition_GetElectricalPositionOffset();
			memcpy(&calibration.Data[4],&off,sizeof(float));
			fullCAN_SendPacket(&calibration,FULLCAN_IDBASE_CALIBRATION);
		}

		vTaskDelayUntil( &xLastWakeTime, DEBUG_PERIOD_MS);
...
static void a2dPortal( int argc, char** argv )
{
	(void) printf("\nIa: %.3f, Ib: %.3f, Ic: %.3f\n",
			...
			analogReadings.Ib,
			analogReadings.Ic);
	(void) printf("Va: %.3f, Vb: %.3f, Vc: %.3f\n",
			analogReadings.Va,
			analogReadings.Vb,
			analogReadings.Vc);
	(void) printf("Vn: %.3f, Vbus: %.3f\n",
			adc_GetReading(ADC_CH_INDEX_VN, FILTER, FILTER_LEN),
			analogReadings.Vbus);
	(void) printf("HWIF: %.3f, VGATE: %.3f\n",
			adc_GetReading(ADC_CH_INDEX_HWIF, FILTER, FILTER_LEN),
			adc_GetReading(ADC_CH_INDEX_VGATE, FILTER, FILTER_LEN));
}
...
static void drivemodePortal( int argc, char** argv )
{
	...
	{
		SetDrivemodeU8( (uint8) strtoul(argv[1],NULL,0) );
	}
}

static void PPOffsetAnglePortal( int argc, char** argv )
{
	DetermineMotorOffset();
}
